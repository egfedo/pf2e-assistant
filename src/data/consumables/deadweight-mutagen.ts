...
export const actions: Assistant.Action[] = [
    {
        trigger: "consume",
        predicate: ["consumable:deadweight-mutagen-lesser"],
    ...
    {
        trigger: "consume",
        predicate: ["consumable:deadweight-mutagen-moderate"],
        process: async (data: Assistant.Data) => {
            if (!data.speaker) return;
            const target = data.target?.actor ?? data.speaker.actor;
    ...
    {
        trigger: "consume",
        predicate: ["consumable:deadweight-mutagen-greater"],
        process: async (data: Assistant.Data) => {
            if (!data.speaker) return;
            const target = data.target?.actor ?? data.speaker.actor;
    ...
    {
        trigger: "consume",
        predicate: ["consumable:deadweight-mutagen-major"],
        process: async (data: Assistant.Data) => {
            if (!data.speaker) return;
            const target = data.target?.actor ?? data.speaker.actor;
