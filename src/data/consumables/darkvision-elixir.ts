...
export const label = "Consumables | Darkvision Elixir";

export const actions: AssistantAction[] = [
    {
        trigger: "consume",
        predicate: ["consumable:darkvision-elixir-lesser"],
    ...
    {
        trigger: "consume",
        predicate: ["consumable:darkvision-elixir-moderate"],
        process: async (message: AssistantMessage) => {
            if (!message.speaker?.actor) return;
            const target = message.target?.actor ?? message.speaker.actor;
    ...
    {
        trigger: "consume",
        predicate: ["consumable:darkvision-elixir-greater"],
        process: async (message: AssistantMessage) => {
            if (!message.speaker?.actor) return;
            const target = message.target?.actor ?? message.speaker.actor;
